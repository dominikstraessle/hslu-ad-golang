...
func OptimalMismatch(text, pattern string) int {
	...
	for {
		...
		mismatchShift := pMismatch[j]
		if text[i+mismatchShift] == pattern[mismatchShift] || pattern[mismatchShift] == wildcard { // match
			j++
		} else {
			if j > 1 {
				pMismatch[j-1], mismatchShift = mismatchShift, pMismatch[j-1]
			}
